...
class Order
{
    ...
    {
        $query = "SELECT o.*, u.username, u.name, u.address, u.phone, p.name AS product_name, o.buy_not 
              FROM orders o 
              JOIN users u ON o.user_id = u.ID 
              JOIN products p ON o.product_id = p.ID 
              WHERE 1=1";
    ...
    public function createOrder($user_id, $product_id, $quantity, $total_price, $phone, $note, $buy_not)
    {
        try {
            ...
            // Insert order with product_name and buy_not
            $stmt = $this->conn->prepare("
                INSERT INTO orders (user_id, product_id, product_name, quantity, total_price, status, phone, note, buy_not, created_at)
                VALUES (:user_id, :product_id, :product_name, :quantity, :total_price, :status, :phone, :note, :buy_not, :created_at)
            ");
            $stmt->execute([
                ':user_id' => $user_id,
